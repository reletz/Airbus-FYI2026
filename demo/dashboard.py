    ...
    for candidate in [Path("data/live_stream.csv"), Path("data/sample.csv")]:
        if candidate.exists():
            ...
            if "strain" in df.columns:
                ...
                return np.tile(single.reshape(-1, 1), (1, 62))
    rng = np.random.default_rng(123)
    return rng.normal(0.0, 1e-3, size=(200, 62))
    ...
    if run_button:
        with st.spinner("Running demo simulation..."):
            ...
            with tab_main:
                ...
                if anomaly_toggle and flagged_clients:
                ...
                if attack_toggle:
                    ...
                    trust_df = pd.DataFrame({"client_id": [f"AC_{i+1:02d}" for i in range(n_clients)], "trust": trust_scores})
